...
## D046 — Mavona owns the inference-facing agent loop

Provider adapters normalize inference events. Rails discovery, tool execution, approvals and evidence remain above the adapter boundary. This supersedes the external-agent launcher shape illustrated in the original vision without changing D001's provider independence.

## D047 — Subscription bridges must preserve the ownership boundary

Subscription access ships only through a provider-supported integration. The official Codex app-server is supported for authentication and rich-client embedding, but it owns conversation turns, tools and approvals. Until an official surface can provide subscription inference beneath Mavona's loop, ChatGPT subscription execution is reported as unsupported. Token extraction is prohibited.

## D048 — Secrets have a separate store and explicit precedence

Provider environment variables take precedence over session-only credentials, which take precedence over Mavona-owned operating-system credential entries. User and project configuration reject secret-shaped keys.

## D049 — Local is an enforced network mode

An adapter marked local may send provider requests only to literal loopback addresses or `localhost`. Local detection never contacts a cloud catalogue and no fallback provider exists.
