...
test('macOS credential writes keep secret bytes out of process arguments',async()=>{
 let command:CredentialCommand|undefined;const store=new SecureCredentialStore('darwin',async c=>{command=c;return {code:0,stdout:''};},()=>'/usr/bin/security');await store.save('openai','private-canary');expect(command?.argv).toEqual(['/usr/bin/security','-i','-q']);expect(command?.stdin).toContain('"private-canary"');expect(command?.argv.join(' ')).not.toContain('private-canary');
});
