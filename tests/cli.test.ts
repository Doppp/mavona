...
test('nested Rails root uses repository-relative context and runs checks inside the selected application',async()=>{
 const root=await mkdtemp(join(tmpdir(),'mavona-cli-nested-'));let paths:string[]=[];const server=Bun.serve({hostname:'127.0.0.1',port:0,async fetch(request){const body=await request.json() as {messages:{content:string}[]};paths=JSON.parse(body.messages[1]!.content).sources.map((source:{path:string})=>source.path);return new Response('data: '+JSON.stringify({choices:[{delta:{content:'Ready for independent checks.'},finish_reason:'stop'}]})+'\n\ndata: [DONE]\n\n',{headers:{'content-type':'text/event-stream'}});}});
 try{await Bun.spawn(['git','init','-q',root]).exited;for(const app of ['shop','admin']){await mkdir(join(root,'apps',app,'config'),{recursive:true});await mkdir(join(root,'apps',app,'app/models'),{recursive:true});await writeFile(join(root,'apps',app,'config/application.rb'),'');await writeFile(join(root,'apps',app,'app/models/order.rb'),'class Order; end');}const argv=JSON.stringify(['/bin/test','-f','app/models/order.rb']);const child=Bun.spawn([...command,'run','Review orders','--root',join(root,'apps/shop'),'--provider','fixture','--model','test','--endpoint',`http://127.0.0.1:${server.port}/v1`,'--locality','local','--trust-model-tools','--allow-command',argv,'--verify',argv,'--data-dir',join(root,'.mavona/data')],{stdout:'pipe',stderr:'pipe'});const result=JSON.parse(await new Response(child.stdout).text());expect(await child.exited).toBe(0);expect(result.correctness).toBe('passed');expect(paths).toEqual(['apps/shop/app/models/order.rb']);}
 finally{server.stop(true);await rm(root,{recursive:true,force:true});}
});
test('task inspection uses packaged no-boot route parsing with declared provenance',async()=>{
 const root=await mkdtemp(join(tmpdir(),'mavona-route-probe-cli-'));await Bun.spawn(['git','init','-q',root]).exited;await mkdir(join(root,'config'));await mkdir(join(root,'app/models'),{recursive:true});await writeFile(join(root,'config/application.rb'),'raise "never boot"');await writeFile(join(root,'app/models/order.rb'),'class Order;end');await writeFile(join(root,'config/routes.rb'),'File.write("PWNED", "bad")\nRails.application.routes.draw { resources :orders }\n');
 try{const ruby=Bun.spawn(['ruby','--disable-gems','-rrbconfig','-e','puts RbConfig.ruby'],{stdout:'pipe',stderr:'pipe'});const executable=(await new Response(ruby.stdout).text()).trim();expect(await ruby.exited).toBe(0);const {dirname}=await import('node:path');const child=Bun.spawn([...command,'inspect',root,'--task','Change order'],{cwd:root,env:{PATH:dirname(executable)+':/usr/bin:/bin',RUBYOPT:'-r/nonexistent/hostile'},stdout:'pipe',stderr:'pipe'});const result=JSON.parse(await new Response(child.stdout).text());expect(await child.exited).toBe(0);expect(result.structure.status).toBe('passed');expect(result.routing.candidates.find((candidate:{path:string})=>candidate.path==='config/routes.rb')).toMatchObject({nominated:true,signal:'declared_route'});expect(result.runtime).toBe('unknown');expect(await Bun.file(join(root,'PWNED')).exists()).toBe(false);}
 finally{await rm(root,{recursive:true,force:true});}
});
