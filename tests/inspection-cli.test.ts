...
test('browser CLI requires exact flow review and emits actual checks and managed report',async()=>{
 ...
 try{const refused=await run([]);expect(refused.code).toBe(2);expect(refused.output.approval.flowDigest).toBe(digestFlow(flow));expect(hits).toBe(0);
  const reviewed=await run(['--trace','--video']);expect(reviewed.code).toBe(2);expect(reviewed.output.approval.flow.evidence).toEqual({trace:true,video:true});expect(reviewed.output.approval.flowDigest).not.toBe(digestFlow(flow));const actual=await run(['--trace','--video','--approve-flow',reviewed.output.approval.flowDigest]);expect(actual.code).toBe(0);expect(actual.output.report.status).toBe('passed');expect(actual.output.report.trace.status).toBe('passed');expect(actual.output.report.video.status).toBe('passed');expect(actual.output.report.checks[0].status).toBe('passed');expect(await Bun.file(actual.output.reportPath).exists()).toBe(true);expect(hits).toBeGreaterThan(0);
