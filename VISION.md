...
Rails engineering meta-harness
    ...
    └── narrow official subscription bridges
            │
            ▼
       Rails repository
```

Mavona is not a generic chat client, Codex wrapper, benchmark-only harness, Rails application, IDE or hosted inference service. Its model loop exists specifically to apply Mavona's Rails discovery, constrained tools and evidence contract consistently across inference routes.
