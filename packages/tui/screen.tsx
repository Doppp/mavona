...
export function Screen(props:{model:()=>ScreenModel;actions:ScreenActions}) {
 const renderer=useRenderer();const dimensions=useTerminalDimensions();const wide=()=>dimensions().width>=120;const theme=()=>terminalTheme(props.model().theme);let input:TextareaRenderable|undefined;const [busy,setBusy]=createSignal(false);
 ...
 return <box flexDirection="column" width="100%" height="100%" paddingX={1} backgroundColor={theme().background}>
  <box height={3} borderStyle="single" border={['bottom']} borderColor={theme().border}><text fg={theme().foreground}><strong>Mavona</strong>  {props.model().repository}  ·  {props.model().connection??'NO PROVIDER'}</text></box>
  ...
  <Show when={props.model().inspection} fallback={<box flexDirection="row" flexGrow={1}>
   <scrollbox id="transcript" visible={!props.model().source||wide()} ref={value=>{transcriptScroll=value;}} stickyScroll={true} stickyStart="bottom" flexGrow={1} width={wide()&&props.model().source?`${100-(props.model().sourcePanePercent??40)}%`:'100%'}><text>{props.model().status}</text><Show keyed when={props.model().theme??'dark'}>{name=><Transcript messages={()=>props.model().messages} theme={()=>name}/>}</Show></scrollbox>
   <Show when={props.model().source}>
    <box flexDirection="column" width={wide()?`${props.model().sourcePanePercent??40}%`:'100%'} flexShrink={0}>
     <text height={2} flexShrink={0}>{props.model().source?.path}{'\n'}{props.model().source?.diff?`Diff ${props.model().source?.diff?.beforeDigest.slice(0,8)} → ${props.model().source?.diff?.afterDigest.slice(0,8)}`:`Source · ${props.model().source?.revision?.startsWith('commit:')?'commit '+props.model().source?.revision?.slice(7,19):'worktree snapshot'}`} · line {props.model().source?.line??1} · {props.model().source?.selection?`selected ${props.model().source?.selection?.start}–${props.model().source?.selection?.end}`:'Esc close'}</text>
     <scrollbox ref={value=>{sourceScroll=value;lastSourcePosition='';}} renderBefore={positionSource} scrollX={!props.model().source?.wrap} flexGrow={1}><Show keyed when={props.model().theme??'dark'}>{name=><SourceBody source={()=>props.model().source!} theme={()=>name}/>}</Show></scrollbox>
    </box>
   </Show>
  ...
  <box borderStyle="single" border={true} borderColor={theme().border} focusedBorderColor={theme().focus} minHeight={4} maxHeight={8}>
   <textarea ref={value=>{input=value;}} focused={!props.model().approval&&!props.model().picker&&!props.model().inspection} initialValue={props.model().draft} placeholder="/help · /files · /open path · /connect provider model" textColor={theme().foreground} backgroundColor={theme().background} cursorColor={theme().focus} selectionBg={theme().selection}
  ...
  <text height={1} fg={theme().muted}>STRICT · {busy()||props.model().running?'working':'idle'} · theme {theme().name} · {props.model().usage??''} · Enter submit · Ctrl+J newline · Ctrl+C twice exit</text>
 </box>;
}
