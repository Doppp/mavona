...
export function Screen(props:{model:()=>ScreenModel;actions:ScreenActions}) {
 ...
 return <box flexDirection="column" width="100%" height="100%" paddingX={1}>
  ...
  <Show when={props.model().picker} fallback={<box height={0}/>}><box border borderStyle="single" flexDirection="column" height={12}><text>Files · type to filter · ↑↓ choose · Enter open · Esc close</text><input value={props.model().picker?.query??''} focused={!!props.model().picker&&!props.model().approval} maxLength={256} onInput={value=>props.actions.filterFiles?.(value)} /><select ref={value=>{picker=value;}} options={props.model().picker?.items.map(item=>({name:item.path,description:'',value:item.id}))??[]} showDescription={false} showScrollIndicator={true} flexGrow={1}/></box></Show>
  <Show when={props.model().source} fallback={<scrollbox id="transcript" ref={value=>{transcriptScroll=value;}} stickyScroll={true} stickyStart="bottom" flexGrow={1}><text>{props.model().status}</text><Transcript messages={()=>props.model().messages}/></scrollbox>}>
  ...
  <box borderStyle="single" border={true} minHeight={4} maxHeight={8}>
   <textarea ref={value=>{input=value;}} focused={!props.model().approval&&!props.model().picker} initialValue={props.model().draft} placeholder="/help · /files · /open path · /connect provider model"
