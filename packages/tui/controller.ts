...
export class TuiController {
 private repairAttempts:0|1=1;model:ScreenModel;verifiers:Verifier[]=[];
 ...
 private select(selection:ProviderSelection){this.selection=selection;this.modelConnection=undefined;this.models=[];this.policy.revoke();this.store.append('provider.configured',{configuration:selectionRecord(selection)});this.update({connection:this.connectionLabel(selection)});}
 ...
 async submit(text:string){
  ...
  try{
   ...
   }else if(text==='/disconnect'){this.selection=undefined;this.models=[];this.store.append('provider.disconnected',{reason:'Explicit user disconnect'});this.update({connection:undefined,picker:undefined});this.policy.revoke();}
