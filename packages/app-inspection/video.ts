...
export class MaskedVideo{
 ...
 async finish(directory:string):Promise<VideoReport>{
  if(this.failed||!this.frames.length){this.frames=[];return {status:'unknown',reason:'Masked video frame budget exceeded or no frames recorded'};}
  ...
  try{const executable=registry.registry.findExecutable('ffmpeg')?.executablePath();if(!executable)throw new Error('Managed FFmpeg unavailable');
   child=Bun.spawn([executable,'-loglevel','error','-f','image2pipe','-framerate','2','-vcodec','mjpeg','-i','pipe:0','-an','-c:v','vp8','-deadline','realtime','-threads','1','-b:v','1M','-t','300','-fs',String(64*1024*1024),output],{stdin:Buffer.concat(this.frames),stdout:'ignore',stderr:'ignore',env:{PATH:'/usr/bin:/bin',LANG:'C'}});
   let timedOut=false;timer=setTimeout(()=>{timedOut=true;child?.kill('SIGKILL');},30000);const exit=await child.exited;if(exit!==0||timedOut)throw new Error('Video encoder unavailable or failed');const bytes=await readFile(output);if(bytes.length<4||bytes.length>=64*1024*1024||bytes.subarray(0,4).toString('hex')!=='1a45dfa3')throw new Error('Invalid or oversized video');
