...
export async function runInspection(options:{root:string;flow:Flow;store:SessionStore;signal:AbortSignal;onEvent?:(event:Envelope)=>void}){
 ...
 try{
  await mkdir(directory,{recursive:true,mode:0o700});const intent=store.append('effect.requested',{effectId,kind:'browser'});options.onEvent?.(intent);
  ...
  const effect=store.append('effect.completed',{effectId,state:signal.aborted||report.state!=='completed'?'unknown':'passed'},intent.eventId);options.onEvent?.(effect);
