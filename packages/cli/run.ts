...
import {readFlow} from './app';import {modelInspectionFlow} from '../agent/inspection-policy';import {digestFlow} from '../app-inspection/service';
...
export async function runCommand(raw:string[]):Promise<number>{
 const args=parseArguments(raw,['provider','model','endpoint','locality','root','format','allow-write','allow-command','verify','max-turns','data-dir','allow-inspection-flow','repair-attempts','allow-runtime-probe','allow-image-sha256'],['trust-model-tools']);
 ...
 const repairAttempts=Number(single(args,'repair-attempts','1'));if(repairAttempts!==0&&repairAttempts!==1)throw new Error('Repair attempts must be zero or one');
 ...
 const imageAuthorization=single(args,'allow-image-sha256');const imageHashes:unknown=imageAuthorization===undefined?[]:JSON.parse(imageAuthorization);if(!Array.isArray(imageHashes)||imageHashes.length>4||(imageAuthorization!==undefined&&!imageHashes.length)||new Set(imageHashes).size!==imageHashes.length||imageHashes.some(hash=>typeof hash!=='string'||! /^[a-f0-9]{64}$/.test(hash)))throw new Error('Image authorization requires one to four distinct SHA-256 digests as JSON');const imageGrants=new Set(imageHashes as string[]);
 const runtimeAuthorization=single(args,'allow-runtime-probe');let runtimeModels:unknown=runtimeAuthorization?JSON.parse(runtimeAuthorization):undefined;if(runtimeModels!==undefined&&(!Array.isArray(runtimeModels)||runtimeModels.length>8||new Set(runtimeModels).size!==runtimeModels.length||runtimeModels.some(name=>typeof name!=='string'||name.length>200||!/^([A-Z]\w*)(::[A-Z]\w*)*$/.test(name))))throw new Error('Runtime probe authorization requires up to eight distinct model names as JSON');
 const approvedInspectionFlows=await Promise.all((args.values.get('allow-inspection-flow')??[]).map(async path=>({path,digest:digestFlow(modelInspectionFlow(await readFlow(path)))})));const inspectionSettings=await settingsDigest(root,inspection.selectedRoot!);
 ...
 let provider=createProvider(providerId,credential?.value,custom);const controller=new AbortController();const cancel=()=>controller.abort();process.on('SIGINT',cancel);process.on('SIGTERM',cancel);
 ...
 try{
  ...
  if(imageGrants.size){capabilities.images=true;capabilities.source='user_override';}
  ...
  const images=imageGrants.size?new TaskImages(store.directory,{connection,model},capabilities):undefined;if(images)provider=createProvider(providerId,credential?.value,custom,images.adapterOptions());
   const result=await runTask({root,railsPath:join(root,inspection.selectedRoot!),task,provider,providerId,model,capabilities,store,policy,signal:controller.signal,verifiers,maxTurns,repairAttempts,...(images?{images,approveImages:async(review:import('../agent/images').ImageReview)=>{if(await settingsDigest(root,inspection.selectedRoot!)!==inspectionSettings||!images.matches(providerId,model,connection.locality))return false;const hashes=review.images.map(image=>image.sha256);if(new Set(hashes).size!==hashes.length||hashes.some(hash=>!imageGrants.has(hash)))return false;for(const hash of hashes)imageGrants.delete(hash);return true;}}:{}),approveRuntimeProbe:async action=>{if(runtimeModels===undefined||await settingsDigest(root,inspection.selectedRoot!)!==inspectionSettings)return false;const request=JSON.parse(action.action.tool==='run_command'?action.action.argv.at(-1)!:'null');if(JSON.stringify(request?.models)!==JSON.stringify(runtimeModels))return false;runtimeModels=undefined;return true;},approveInspection:async review=>{const index=approvedInspectionFlows.findIndex(item=>item.digest===review.flowDigest);if(index<0||await settingsDigest(root,inspection.selectedRoot!)!==inspectionSettings)return false;const approved=approvedInspectionFlows[index]!;if(digestFlow(modelInspectionFlow(await readFlow(approved.path)))!==approved.digest)return false;approvedInspectionFlows.splice(index,1);return true;},onEvent:event=>{if(format==='jsonl')console.log(JSON.stringify(event));else if(format==='text'&&event.type==='assistant.delta')process.stdout.write(String(event.payload.text));}});
