...
export async function runCommand(raw:string[]):Promise<number>{
 const args=parseArguments(raw,['provider','model','endpoint','locality','root','format','allow-write','allow-command','verify','max-turns','data-dir','allow-inspection-flow','repair-attempts'],['trust-model-tools']);
