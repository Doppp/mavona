...
// Secret-bearing stdin is private; commands and stdin must never be logged.
...
export class SecureCredentialStore {
 ...
 async save(provider:string,value:string):Promise<void>{const account=this.account(provider);if(!value||value.length>16384||value.includes('\0')||/[\r\n]/.test(value))throw new SecureStoreError('Invalid credential');const cmd=this.command();const result=await this.execute(this.platform==='darwin'?[cmd,'-i','-q']:[cmd,'store','--label=Mavona provider credential','service','com.mavona.credentials','account',account],this.platform==='darwin'?['add-generic-password','-U','-s','com.mavona.credentials','-a',account,'-w',value].map(securityArgument).join(' ')+'\n':value);if(result.code!==0)throw new SecureStoreError('Operating-system secure storage save failed; use environment or session credentials explicitly');}
...
// security(1) interactive tokenizer quoting; no shell is invoked.
export function securityArgument(value:string):string{if(/[\0\r\n]/.test(value))throw new SecureStoreError('Invalid secure-store argument');return '"'+value.replace(/\\/g,'\\\\').replace(/"/g,'\\"')+'"';}
