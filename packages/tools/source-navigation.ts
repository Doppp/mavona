...
export interface SourceView extends SourceSnapshot {line:number;wrap:boolean;query:string;matches:number[];selection?:SourceSelection}
export class SourceNavigator {
 ...
 goto(line:number){const source=this.required();if(!Number.isSafeInteger(line)||line<1||line>source.lineCount)throw new Error('Line outside source');this.current={...source,line};}
 search(query:string){if(!query||query.length>200)throw new Error('Search needs 1–200 literal characters');const source=this.required();const matches=source.text.split('\n').flatMap((line,i)=>line.includes(query)?[i+1]:[]);this.current={...source,query,matches,line:matches.find(n=>n>=source.line)??matches[0]??source.line};}
 ...
 select(start:number,end:number){const source=this.required();this.current={...source,selection:selectLines(source,start,end)};}
