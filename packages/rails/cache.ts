...
export interface StructuralObservation {path:string;digest:string;capturedAt:string;provenance:'fresh'|'cache';querySchemaVersion:2}
...
export async function parseRubyFilesCached(root:string,paths:string[],cacheDirectory:string,options:ProbeOptions={}):Promise<CachedRubyStructure>{
 ...
 for(const path of paths){
  const source=await readSource(repository,path);const key=digest(JSON.stringify([2,repository,source.path,source.digest,PARSER_VERSION]));let reused=false;
  try{
   const entry:unknown=JSON.parse((await readSource(cache,`${key}.json`,2*1024*1024)).text);
   if(typeof entry==='object'&&entry!==null&&'schemaVersion'in entry&&entry.schemaVersion===2&&'parserVersion'in entry&&entry.parserVersion===PARSER_VERSION&&'capturedAt'in entry&&typeof entry.capturedAt==='string'&&Number.isFinite(Date.parse(entry.capturedAt))&&'file'in entry&&validateRubyFileFacts(entry.file,source.path,source.digest,source.lineCount)){
    files.push(entry.file);observations.push({path:source.path,digest:source.digest,capturedAt:entry.capturedAt,provenance:'cache',querySchemaVersion:2});reused=true;
   }
  ...
  const capturedAt=new Date().toISOString();files.push(file);observations.push({path:file.path,digest:file.digest,capturedAt,provenance:'fresh',querySchemaVersion:2});
  const temporary=join(cache,`${key}.${randomUUID()}.tmp`);
  try{await writeFile(temporary,JSON.stringify({schemaVersion:2,parserVersion:PARSER_VERSION,capturedAt,file}),{flag:'wx',mode:0o600});await rename(temporary,join(cache,`${key}.json`));}finally{await rm(temporary,{force:true});}
 }
