...
import os,pty,subprocess,tempfile,pathlib,select,time,termios,fcntl,struct,sys,signal,json,threading,http.server,urllib.request,urllib.error
...
with tempfile.TemporaryDirectory(prefix='mavona-live-inspection-pty-') as directory:
 root=pathlib.Path(directory);(root/'config').mkdir();(root/'config/application.rb').write_text('');(root/'bin').mkdir();rails=root/'bin/rails';rails.write_text("""#!/usr/bin/ruby
require 'socket'
port=ARGV[ARGV.index('--port')+1].to_i
puts 'owned server ready';STDOUT.flush
server=TCPServer.new('127.0.0.1',port)
trap('TERM'){server.close rescue nil;exit}
loop do
 socket=server.accept
 socket.readpartial(8192) rescue nil
 body='<h1>Owned Rails server</h1>'
 socket.write("HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: #{body.bytesize}\r\nConnection: close\r\n\r\n#{body}")
 socket.close
end
""");rails.chmod(0o700);cache=root/'Library/Caches';cache.mkdir(parents=True);(cache/'ms-playwright').symlink_to(pathlib.Path.home()/'Library/Caches/ms-playwright');subprocess.run(['git','init','-q',directory],check=True);server=http.server.ThreadingHTTPServer(('127.0.0.1',0),App);threading.Thread(target=server.serve_forever,daemon=True).start();url='http://127.0.0.1:'+str(server.server_port)
 ...
 try:
  ...
  print('PASS native live inspection: reviewed attach and discovered start, capture, takeover, fresh resume, ownership-correct stop, draft, resize and cleanup')
 finally:
