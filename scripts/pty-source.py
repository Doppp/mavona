...
with tempfile.TemporaryDirectory(prefix='mavona-source-pty-') as directory:
 root=pathlib.Path(directory);(root/'config').mkdir();(root/'config/application.rb').write_text('');(root/'app/models').mkdir(parents=True);(root/'app/models/order.rb').write_text('# 注文\nclass Order\nend\n')
 ...
 try:
  collect(1);os.write(master,b'unfinished task');collect(.2);os.write(master,b'\x10');collect(.2);os.write(master,b'amord');collect(.2);os.write(master,b'\r');collect(.6)
