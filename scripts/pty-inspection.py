"""Native actual browser evidence → resumed text-only drawer at 80×24."""
import os,pty,subprocess,tempfile,pathlib,select,time,termios,fcntl,struct,sys,signal,json,threading,http.server
...
with tempfile.TemporaryDirectory(prefix='mavona-inspection-pty-') as directory:
 ...
 try:
  ...
  print('PASS native inspection drawer: actual capture, resume, 80×24 text fallback, scrolling, draft focus, resize, no replay, terminal cleanup')
 finally:
