...
| MODEL-04 | implementing | Official subscription capability assessed; explicit unsupported own-loop state implemented without token extraction. |
| APP-01 | implementing | Real lifecycle, isolated auth, interactions, scoped coding-loop flow and live terminal attach/capture/takeover/resume/stop implemented; approved server-start UI and full lifecycle matrix pending. |
...
Actual compiled macOS arm64 comparison/image/drawer suite:6passed,58assertions,0failed. A real layout change produced a positive difference; an actual inspection-service browser checked the heading, overlay toggle and hidden side-by-side image. Canonical-ID picker, draft preservation, read-only CLI summary and tamper refusal passed. Native pty-inspection-comparison.py passed two real captures, exact IDs, unknown truth, local viewer, no replay/inference, draft, resize and shutdown. Viewer URLs now occupy their own line at80columns after the PTY check exposed split raw terminal output. Strict typecheck, native build and diff check passed. Reviewed baseline integration, measured repeatability and full App Inspection acceptance remain implementing.
...
Authentication state is stored with private permissions outside Git, session artifacts, events, reports and exports, expires within 24 hours, and requires a fresh exact same-origin reuse review. Loading now revalidates exclusion from the current artifact root as well as saving. Actual focused browser tests: 24 passed / 123 assertions / 0 failed across live TUI, authentication, lifecycle and advanced multi-engine behavior. Native compiled `python3 scripts/pty-inspection-live.py dist/mavona` passed reviewed attach, masked capture, takeover, fresh resume, owned-browser shutdown, attached-server preservation, draft retention, resize and terminal cleanup. Strict typecheck and native build passed. Approved discovered server start/health/log controls, public authentication revocation, and the remaining 14-case release matrix still require acceptance.
