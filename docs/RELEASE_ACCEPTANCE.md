...
### Canonical scenario A — continuous Rails feature pipeline

A temporary committed copy of the canonical Rails fixture began with three deliberate rescheduling defects: missing future-date validation, unscoped order lookup and disconnected Stimulus form behavior. One deterministic local provider used the production agent loop to read and digest-bind patches to the model, controller and view. The same task then launched the actual isolated Rails server, exercised invalid input, corrected input, Stimulus feedback, Turbo confirmation and reload persistence, and ran `ruby bin/check` as an independent required verifier. The verifier includes invalid-date and cross-customer authorization outcomes. A pre-change and post-change managed capture produced a positive diagnostic difference. Closing and reopening canonical session history retained a passed task result, allowed mutation and no unknown effects. `bun test tests/canonical-scenario-a.test.ts`: 1 passed / 12 assertions / 0 failed in 6.10 seconds on macOS arm64 with Ruby 4.0.6 and the pinned fixture. Live cloud/local-model demonstrations and other-host execution remain separate release requirements.

### Canonical scenario B — focused Rails repair

A copied Rails fixture adds a failing `Admin::Order` Minitest and a broken namespaced model. The first actual `ruby bin/rails test test/models/admin/order_test.rb` result supplies bounded failure output with the test identity and file/line to the one allowed repair pass. The deterministic provider reads and digest-patches only the model; the same focused Rails command then passes. One `task.repair.started` event and two verifier results prove the bounded retry. This test exposed command identity scanning incorrectly treating the ordinary Rails subcommand word `test` as a source file when the `test/` directory existed. The policy now skips contained directory arguments while continuing to bind regular entry files and reject containment escapes. Scenario B plus policy/repair regression: 17 tests / 64 assertions / 0 failed; the separate persistent-failure case still stops after one repair. Other-host execution remains pending.

### Canonical scenario C — editor intervention and proposal drift

A committed fixture receives an unrelated pre-existing user edit before a digest-bound agent patch is prepared. An actual editor handoff holds worktree ownership, changes the proposal target, restores the terminal lifecycle and reports both the editor change and the pre-existing path. The previously prepared patch is then refused as stale without replacing the editor bytes. After a fresh read and exact approval, the intended patch applies and an independent subprocess checks both the new target state and preserved unrelated edit. `bun test tests/canonical-scenario-c.test.ts` is the continuous scenario; native Vim PTY and wait-capable GUI handoff tests separately cover their actual adapters and cleanup.
