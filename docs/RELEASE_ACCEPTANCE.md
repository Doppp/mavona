...
| MODEL-04 | implementing | Official subscription capability assessed; explicit unsupported own-loop state implemented without token extraction. |
| APP-01 | implementing | Real lifecycle, isolated auth, interactions, scoped coding-loop flow and live terminal attach/start/log/capture/takeover/resume/stop implemented; full lifecycle/recovery matrix pending. |
